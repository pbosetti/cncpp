...
In the **linux console** (i.e. the terminal), install the following packages:

```sh
sudo apt install libcurl-openssl-dev libfontconfig1-dev libharfbuzz-dev libfribidi-dev libfreetype6-dev libpng-dev libtiff5-dev libjpeg-dev
```

And install the CNCpp library (still in the terminal from the CNCpp project folder):
...
This project uses `renv` to manage dependencies. To install the required packages, run the following command in the R console (copy and paste, do not execute the block!):
...
cnc$load_file("../project_path.gcode")
path <- cnc$simulate() %>% 
  mutate(n=factor(n))
...
This is for testing the TRC (Tool Radius Compensation) feature of the CNCpp library.

```{r}
cnc <- new(CNCpp, "../machine.yml")
prog <-  list(
  "N10 G00 X-30 Y0 Z300 T01 # Load tool 1 with radius 3 mm", 
  "N20 G00 Z50", 
  "N30 G01 Z20 F500", 
...
cnc$load(prog)
# Or, load the file
# cnc$load_file("../trc.gcode")
path <- cnc$simulate() %>% 
  mutate(n=factor(n))

path %>% ggplot(aes(x=x, y=y, color=n)) + 
  geom_path() +
  coord_equal() +
  labs(x="X (mm)", y="Y (mm)")
```
