...
gv_fsm version 0.4.1
Generation date: 2024-10-17 14:53:46 +0200
  ...
  void install_functions() {
    ...
    add_state(FSM::STATE_LOAD_BLOCK, [](DATA_T &data) -> FSM::state_t {
      ...
      case FSM::STATE_INTERP_MOTION:
        break;
      default:
        syslog(LOG_WARNING, "[FSM] Cannot pass from load_block to %s, remaining in this state", state_names[next_state]);
        next_state = FSM::NO_CHANGE;
      }
      return next_state;
    });
    ...
    add_state(FSM::STATE_GO_TO_ZERO, [](DATA_T &data) -> FSM::state_t {
      ...
      case FSM::STATE_GO_TO_ZERO:
        break;
      default:
        syslog(LOG_WARNING, "[FSM] Cannot pass from go_to_zero to %s, remaining in this state", state_names[next_state]);
        next_state = FSM::NO_CHANGE;
      }
      // SIGINT transition override
      if (stop_requested) next_state = STATE_STOP;
    ...
    add_state(FSM::STATE_NO_MOTION, [](DATA_T &data) -> FSM::state_t {
      ...
      case FSM::STATE_LOAD_BLOCK:
        break;
      default:
        syslog(LOG_WARNING, "[FSM] Cannot pass from no_motion to %s, remaining in this state", state_names[next_state]);
        next_state = FSM::NO_CHANGE;
      }
      return next_state;
    });
    ...
    add_state(FSM::STATE_RAPID_MOTION, [](DATA_T &data) -> FSM::state_t {
      ...
      case FSM::STATE_RAPID_MOTION:
        break;
      default:
        syslog(LOG_WARNING, "[FSM] Cannot pass from rapid_motion to %s, remaining in this state", state_names[next_state]);
        next_state = FSM::NO_CHANGE;
      }
      // SIGINT transition override
      if (stop_requested) next_state = STATE_STOP;
    ...
    add_state(FSM::STATE_INTERP_MOTION, [](DATA_T &data) -> FSM::state_t {
      ...
      case FSM::STATE_INTERP_MOTION:
        break;
      default:
        syslog(LOG_WARNING, "[FSM] Cannot pass from interp_motion to %s, remaining in this state", state_names[next_state]);
        next_state = FSM::NO_CHANGE;
      }
      // SIGINT transition override
      if (stop_requested) next_state = STATE_STOP;
